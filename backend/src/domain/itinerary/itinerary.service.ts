...
import {
  CreateItineraryActivityPayload,
  ItineraryDay,
} from "./itinerary.entity";
...
const getDateOnlyInMexico = (value?: string | null): string | null => {
...
const assertItineraryMutationIsNotInPast = (value?: string | null): void => {
  const dateOnly = getDateOnlyInMexico(value);
  if (dateOnly && dateOnly < getTodayDateOnly()) {
    throw Object.assign(
      new Error(
        "No puedes crear o modificar actividades en dias anteriores al actual.",
      ),
      { statusCode: 400 },
    );
  }
...
const resolveActivityDateTime = (
  ...
  const proposalPayload =
    ...
    typeof activity.propuestas.payload === "object"
      ? activity.propuestas.payload
      : {};
...
const ensureNoGroupActivityCollision = async ({
  ...
  const conflictingActivity = (siblingActivities ?? []).find(
    (activity: any) => {
      if (
        excludeActivityId &&
        String(activity.id_actividad) === String(excludeActivityId)
      )
        return false;
      ...
      const siblingMinuteKey = getScheduleMinuteKey(
        activity.fecha_inicio ? String(activity.fecha_inicio) : null,
      );
      if (candidateMinuteKey && siblingMinuteKey === candidateMinuteKey)
        return true;

      return candidateRange && siblingRange
        ? rangesOverlap(candidateRange, siblingRange)
        : false;
    },
  );
  ...
  if (conflictingActivity) {
    throw Object.assign(
      new Error(
        `Ya existe una actividad grupal que se empalma con este horario: "${String(conflictingActivity.titulo ?? "Actividad existente")}"`,
      ),
      { statusCode: 409 },
    );
  }
  ...
  const conflictingSlot = (subgroupSlots ?? []).find((slot: any) => {
    ...
    return candidateRange && slotRange
      ? rangesOverlap(candidateRange, slotRange)
      : false;
  });
  ...
  if (conflictingSlot) {
    throw Object.assign(
      new Error(
        `Este horario se empalma con el bloque de subgrupos "${String((conflictingSlot as any).title ?? "Horario de subgrupos")}"`,
      ),
      { statusCode: 409 },
    );
  }
...
const getActivityCategory = (
  activity: any,
): "transporte" | "hospedaje" | "actividad" => {
...
export const getGroupItinerary = async (
  ...
  const pendingProposalIds = (activities ?? [])
    .filter(
      (activity: any) =>
        activity.estado === "pendiente" && activity.propuesta_id != null,
    )
  ...
  if (pendingProposalIds.length > 0) {
    ...
    if (
      votesByProposalError &&
      isMissingVoteTypeColumnError(votesByProposalError)
    ) {
      const fallback = await supabase
        .from("voto")
        .select("id_propuesta")
        .in("id_propuesta", pendingProposalIds);
      votesByProposal = (fallback.data ?? []).map((row: any) => ({
        ...row,
        voto_tipo: "a_favor",
      }));
    ...
    if (proposalIdsToConfirm.length > 0) {
      ...
      if (confirmActivitiesError)
        throw new Error(confirmActivitiesError.message);
  ...
  const { data: myVotes, error: myVotesError } =
  ...
  for (const activity of activities ?? []) {
    ...
    days[dayIndex].activities.push({
      ...
      createdBy: String(
        activity.creado_por ?? (activity.propuestas as any)?.creado_por ?? "",
      ),
...
export const createGroupActivity = async (
  ...
  const { data: existingItinerary, error: itineraryLookupError } =
    await supabase
      .from("itinerarios")
      .select("*")
      .eq("grupo_id", groupId)
  ...
  if (!itinerary) {
    const { data: createdItinerary, error: createItineraryError } =
      await supabase
        .from("itinerarios")
        .insert({
    ...
    if (createItineraryError || !createdItinerary) {
      throw new Error(
        createItineraryError?.message ?? "Error al crear itinerario",
      );
    }
  ...
  if (uniqueMembersCount <= 1) {
    ...
    ({ error: voteInsertError } = await supabase.from("voto").insert({
    ...
    if (voteInsertError && isMissingVoteTypeColumnError(voteInsertError)) {
      const fallbackInsert = await supabase.from("voto").insert({
        id_propuesta: proposal.id_propuesta,
        id_usuario: Number(usuarioId),
      });
...
export const updateGroupActivity = async (
  ...
  if (itineraryError) throw new Error(itineraryError.message);
  if (!itinerary)
    throw Object.assign(new Error("Itinerario no encontrado"), {
      statusCode: 404,
    });
  ...
  const { data: currentActivity, error: currentActivityError } = await supabase
    .from("actividades")
    .select(
      "id_actividad, estado, propuesta_id, creado_por, titulo, descripcion, ubicacion, latitud, longitud, fecha_inicio, fecha_fin",
    )
  ...
  const isOwner =
    String((currentActivity as any).creado_por) === String(usuarioId);
  const isAdmin =
    String((membership as any).rol ?? "").toLowerCase() === "admin";
  const currentActivityDate = getDateOnlyInMexico(
    (currentActivity as any).fecha_inicio ?? null,
  );
  const isCurrentActivityPastDay = Boolean(
    currentActivityDate && currentActivityDate < getTodayDateOnly(),
  );

  if (!isOwner && !(isAdmin && isCurrentActivityPastDay)) {
    throw Object.assign(
      new Error(
        isCurrentActivityPastDay
          ? "Solo el administrador puede reprogramar propuestas vencidas"
          : "Solo el creador de la propuesta puede editarla",
      ),
      {
        statusCode: 403,
      },
  ...
  const updateData = {
    ...
    ...(payload.fecha_fin !== undefined
      ...
      : {}),
    ...(payload.referencia_externa !== undefined
  ...
  const titleChanged =
    ...
    toComparable(payload.titulo) !==
      toComparable((currentActivity as any).titulo);
  const descriptionChanged =
    payload.descripcion !== undefined &&
    toComparable(payload.descripcion) !==
      toComparable((currentActivity as any).descripcion);
  const locationChanged =
    payload.ubicacion !== undefined &&
    toComparable(payload.ubicacion) !==
      toComparable((currentActivity as any).ubicacion);
  const latChanged =
  ...
  const startChanged =
    payload.fecha_inicio !== undefined &&
    toComparable(payload.fecha_inicio) !==
      toComparable((currentActivity as any).fecha_inicio);
  const endChanged =
    payload.fecha_fin !== undefined &&
    toComparable(payload.fecha_fin) !==
      toComparable((currentActivity as any).fecha_fin);
  ...
  if (
    currentActivity.estado === "confirmada" &&
    touchedVotingFields &&
    currentActivity.propuesta_id
  ) {
    const nowIso = new Date().toISOString();
...
export const deleteGroupActivity = async (
  ...
  const { usuarioId, membership } = await ensureGroupMember(
    authUserId,
    groupId,
  );
  ...
  const { data: activity, error: activityLookupError } = await supabase
    .from("actividades")
    .select(
      "id_actividad, propuesta_id, creado_por, titulo, ubicacion, fecha_inicio",
    )
    .eq("id_actividad", activityId)
    .eq("itinerario_id", itinerary.id_itinerario)
    .maybeSingle();
  ...
  if (!isOwner && !isAdmin) {
    throw Object.assign(
      new Error(
        "Solo el creador o un administrador del grupo puede eliminarla",
      ),
      {
        statusCode: 403,
      },
  ...
  assertItineraryMutationIsNotInPast((activity as any).fecha_inicio ?? null);
