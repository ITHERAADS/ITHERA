...
function normalizeRealtimeComment(raw: unknown): ProposalComment | null {
  ...
  return {
    ...
    updatedAt: String(
      record.updatedAt ?? record.updated_at ?? new Date().toISOString(),
    ),
  };
}
...
export function ProposalDetailModal({
  ...
  useEffect(() => {
    ...
    if (!isOnline) {
      setVoteSuccess("");
      setVoteError("Sin conexión. Reconecta para votar en la propuesta.");
      return;
    }
    setLoading(true);
    proposalsService
  ...
  useEffect(() => {
    ...
    const belongsToThisProposal = (payload: ProposalCommentSocketPayload) =>
    ...
    const handleCreated = (payload: ProposalCommentSocketPayload) => {
      ...
      setComments((prev) =>
        prev.some((item) => item.id === comment.id) ? prev : [...prev, comment],
      );
    };
  ...
  const chips = [
    ...
    proposal.votes !== undefined && {
      icon: <IconThumbUp />,
      label: `${proposal.votes} votos`,
    },
  ].filter(Boolean) as { icon: React.ReactNode; label: string }[];

  async function handleSendComment() {
    const text = draft.trim();
    if (!text || !proposal.proposalId || !accessToken || !isOnline) return;
    try {
      setSending(true);
      const res = await proposalsService.addComment(
        tripId,
        proposal.proposalId,
        { contenido: text },
        accessToken,
      ...
      setComments((prev) =>
        prev.some((item) => item.id === res.comment.id)
          ? prev
          : [...prev, res.comment],
      );
      setDraft("");
  ...
  async function handleVote(voto: "a_favor" | "en_contra") {
    ...
    try {
      ...
      setVoteSuccess(
        voto === "a_favor" ? "¡Votaste a favor!" : "Votaste en contra.",
      );
    } catch (e) {
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
          ...
          {proposal.proposedBy && (
            <p className="font-body text-xs text-[#1E0A4E]/50 italic">
              Propuesto por{" "}
              <span className="font-semibold not-italic text-[#1E0A4E]/70">
                {proposal.proposedBy}
              </span>
            </p>
          )}
          ...
          <div className="flex flex-col gap-3">
            <h3 className="font-heading font-bold text-[#1E0A4E] text-sm">
              Comentarios
            </h3>
              ...
              <div className="flex flex-col gap-3">
                {comments.map((comment) => (
                  <div key={comment.id} className="flex gap-3">
                    ...
                    >
                      {getInitials(
                        comment.authorName || `U${comment.usuarioId}`,
                      )}
                    </div>
                    <div className="flex-1 bg-[#F8FAFC] rounded-xl px-3 py-2">
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        ...
                        <p className="font-body text-[10px] text-[#1E0A4E]/40">
                          {timeAgo(comment.createdAt)}
                        </p>
                      </div>
                      <p className="font-body text-xs text-[#1E0A4E]/70">
                        {comment.contenido}
                      </p>
                    </div>
                  </div>
